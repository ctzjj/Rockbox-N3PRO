...
sub buildzip {
    ...
    # n3pro: ship the web control frontend (served from .rockbox/web).
    # NB: $modelname arrives wrapped in literal quotes for the zip
    # targets ("-m \"$(MODELNAME)\"" in root.make), so match a substring.
    if ($modelname =~ /n3pro/) {
        if (-d "$ROOT/web/control") {
