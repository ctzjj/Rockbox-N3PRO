...
static void update_playing_time(void)
{
    ...
    /* A streamed source may not carry a bitrate at all (and the id3 is
     * shared with the caller, which can reset it): never divide by zero. */
    if (ci->id3->bitrate)
        ci->set_elapsed((unsigned long)((ci->id3->offset -
                        ci->id3->first_frame_offset) * 8LL /
                        ci->id3->bitrate));
#else
