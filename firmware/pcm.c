...
static void pcm_wait_for_init(void)
{
    ...
    /* sleep(0) is a cooperative yield that a raw pthread never gets woken
     * from; the dedicated poll thread handles initialisation, so a caller
     * must not spin waiting here. */
    (void)0;
#else
...
#if defined(CAYIN_N3PRO)
    /* A fixed-rate output (bluetooth) can only run at the rate it was
     * opened with; clamp to it and let the DSP resample everything else. */
    {
        unsigned int fixed = pcm_sink_fixed_rate();
        if (fixed)
            samplerate = fixed;
    }
#endif
