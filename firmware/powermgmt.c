...
#endif
#if defined(CAYIN_N3PRO)
        /* Non-local sources (network radio, Bluetooth/USB input) do not
         * set the local play status; treat any active content channel as
         * "playing" so the idle timer cannot shut them down. */
        || pcm_mixer_content_playing()
#endif
    ) {
        last_event_tick = current_tick;
    }
