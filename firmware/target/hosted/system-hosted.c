...
void system_init(void)
{
   ...
   /* catch some signals for easier debugging */
    ...
    /* A peer that goes away must not kill the whole player: this port
     * streams audio over TCP and TLS (where MSG_NOSIGNAL is not
     * available), so let the writes fail with EPIPE instead. */
