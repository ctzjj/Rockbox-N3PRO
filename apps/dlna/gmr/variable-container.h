...
// Retarget the collector at a (new) device, or NULL it while the device
// is down.  The renderer can be restarted in this port while the collector
// (and its variable callback) is process-global.
void UPnPLastChangeCollector_set_device(upnp_last_change_collector_t *object,
					struct upnp_device *upnp_device);
