...
static int replace_transport_uri_and_meta(const char *uri, const char *meta) {
	...
	if (meta != NULL) {
		...
		if (t != NULL) {
			...
			if (e != NULL) {
				size_t n = (size_t)(e - t);
				if (n > sizeof(last_push_title_) - 1)
					n = sizeof(last_push_title_) - 1;
...
void upnp_transport_init(struct upnp_device *device) {
	/* The renderer is restartable in this port, but this collector (and
	 * its variable callback) is process-global and tied to the persistent
	 * state_variables_: on a restart retarget it at the new device rather
	 * than assert / create a duplicate (upstream only inits once). */
	if (transport_service_.last_change != NULL) {
		UPnPLastChangeCollector_set_device(
			transport_service_.last_change, device);
		return;
	}
	...
	/* The time updater is an endless thread; create it at most once
	 * (this init runs again whenever the renderer is restarted) and
	 * detach it so a restart cannot leak another one. */
	static int updater_started = 0;
	if (!updater_started) {
		pthread_t thread;
		if (pthread_create(&thread, NULL, thread_update_track_time,
				   NULL) == 0) {
...
/* Detach the collector from the (about to be destroyed) device: no events
 * may be sent while the renderer is down.  Also reset the transport to
 * STOPPED so a restarted renderer accepts the controller's Play again
 * (transport_state_ is process-global and survives the restart). */
