...
void upnp_control_init(struct upnp_device *device) {
	...
	if (control_service_.last_change != NULL) {
		/* restart: retarget the process-global collector instead of
		 * asserting / creating a duplicate (see upnp_transport.c) */
		UPnPLastChangeCollector_set_device(
			control_service_.last_change, device);
		return;
	}
...
/* Detach the collector from the (about to be destroyed) device. */
